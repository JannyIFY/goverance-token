...
(define-public (cast-vote (proposal-id uint) (vote-type uint))
  (let ((proposal (unwrap! (get-proposal proposal-id) error-proposal-not-found))
    ...
    (begin
      ...
      (asserts! (is-eq (get status proposal) status-active) error-proposal-not-active)
      (asserts! (< block-height (get end-block proposal)) error-proposal-expired)
      
      ;; Check vote type is valid
      (asserts! (is-valid-vote-type vote-type) error-invalid-vote-type)
      
      ;; Check voter has tokens
      (asserts! (> voter-balance u0) error-insufficient-tokens)
      
      ;; Check voter hasn't already voted
      (asserts! (is-none (get-vote proposal-id tx-sender)) error-already-voted)
      
      ;; Record the vote
      (map-set votes
        { proposal-id: proposal-id, voter: tx-sender }
        { weight: voter-balance, vote-type: vote-type, time: block-height }
      )
      ...
      (map-set proposals
        ...
        (merge proposal
          (if (is-eq vote-type vote-type-for)
            { vote-for: (+ (get vote-for proposal) voter-balance) }
            (if (is-eq vote-type vote-type-against)
              { vote-against: (+ (get vote-against proposal) voter-balance) }
              { vote-abstain: (+ (get vote-abstain proposal) voter-balance) }
            )
          )
        )
      )
...
;; Cancel a proposal (only by creator)
(define-public (cancel-proposal (proposal-id uint))
  (let ((proposal (unwrap! (get-proposal proposal-id) error-proposal-not-found)))
    (begin
      ;; Check sender is proposal creator
      (asserts! (is-eq tx-sender (get creator proposal)) error-not-proposal-creator)
      ...
      (map-set token-balances 
        tx-sender 
        (+ (get-token-balance tx-sender) (var-get proposal-fee))
      )
...
(define-public (finalize-proposal (proposal-id uint))
  (let ((proposal (unwrap! (get-proposal proposal-id) error-proposal-not-found)))
    (begin
      ...
      (asserts! (>= block-height (get end-block proposal)) error-proposal-not-expired)
      
      ;; Calculate new status
      (let ((new-status (calculate-proposal-status proposal-id)))
        ;; Update proposal status
        (map-set proposals
          { proposal-id: proposal-id }
